...
{
    unsigned numberOfPoints =  ( _numberOfSegments + 1 ) * _pointsPerSegment;
    if ( _samplePoints )
...
void ActivationPatternDivider::allocateMemory()
{
    ...
    _samplePoints = new double*[numberOfPoints];
    for ( unsigned i = 0; i < numberOfPoints; i++ )
        {
    ...
    _patterns = new ActivationPattern*[numberOfPoints];
    for ( unsigned i = 0; i < numberOfPoints; i++ )
                                                        ...
                                                        &inputRegion )
{
    ...
    for ( const auto &variable : _inputVariables )
    {
        if ( samplePoints( inputRegion, variable ) )
        {
            ...
            double variance = getPatternVariance();
...
{
    unsigned numberOfPoints = _numberOfSegments * _pointsPerSegment;
    for ( unsigned i = 0; i < numberOfPoints; i++ )
...
unsigned ActivationPatternDivider::getPatternVariance()
{
    ...
    for ( unsigned i = 0; i < _pointsPerSegment; i++ )
    {
        ...
        for ( unsigned j = 0; _numberOfSegments; j++ )
