 /*********************                                                        */
/*! \file DeepPolyCosineElement.cpp
 ** \verbatim
...
DeepPolyCosineElement::DeepPolyCosineElement( Layer *layer )
{
    _layer = layer;
    _size = layer->getSize();
...
DeepPolyCosineElement::~DeepPolyCosineElement()
{
    freeMemoryIfNeeded();
}

void DeepPolyCosineElement::execute( const Map<unsigned, DeepPolyElement *>
                               &deepPolyElementsBefore )
{
    ...
    for ( unsigned i = 0; i < _size; ++i )
    {
        ...
        CosineConstraint::findRangeOfCosOutput(sourceLb, sourceUb, _lb[i], _ub[i]);
...
void DeepPolyCosineElement::symbolicBoundInTermsOfPredecessor
  ...
  unsigned targetLayerSize, DeepPolyElement *predecessor )
{
    ...
    /*
      ...
      Cosine outputs, the goal is to compute the symbolic bound of the target
      layer in terms of the Cosine inputs.
    */
    for ( unsigned i = 0; i < _size; ++i )
    {
        ...
        */
        ...
        // Symbolic bounds of the Cosine output in terms of the Cosine input
        ...
        // Substitute the Cosine input for the Cosine output
...
void DeepPolyCosineElement::allocateMemory()
{
    freeMemoryIfNeeded();
...
void DeepPolyCosineElement::freeMemoryIfNeeded()
{
...
void DeepPolyCosineElement::log( const String &message )
{
    if ( GlobalConfiguration::NETWORK_LEVEL_REASONER_LOGGING )
        printf( "DeepPolyCosineElement: %s\n", message.ascii() );
}
